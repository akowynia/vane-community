...
export type OllamaCloudLLMConfig = {
  apiKey: string;
  baseURL?: string;
  model: string;
...
export class OllamaCloudLLM extends BaseLLM<OllamaCloudLLMConfig> {
  ollamaClient: Ollama;

  constructor(protected config: OllamaCloudLLMConfig) {
    super(config);
    ...
    const res = await this.ollamaClient
      ...
      .catch((err: any) => {
        if (err?.message?.includes('does not support tools')) {
          throw new Error(
            `The selected Ollama Cloud model "${this.config.model}" does not support tools/function calling.`,
          );
        }
        throw err;
      });
  ...
  async generateObject<T>(input: GenerateObjectInput): Promise<T> {
    ...
    try {
      return parseAndValidateObject<T>(rawContent, input.schema, 'Ollama Cloud');
    } catch (err) {
      throw new Error(`Error parsing response from Ollama Cloud: ${err}`);
    }
  }
